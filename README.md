...
* [`src/snake/styles.cljs`](src/snake/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/snake/`](src/snake/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/snake/core.cljs): contains the SPA entry point, `init`
* [`test/snake/`](test/snake/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `snake` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/snake/config.cljs) defaults to `true` in
...
```clj
(ns snake.example
  (:require [snake.config :as config])
